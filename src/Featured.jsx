// Featured projects + project visuals
const projectVisuals = {
  prism: () => (
    <svg viewBox="0 0 400 500" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" style={{display:'block'}}>
      ...
      {/* prism refraction */}
      <polygon points="200,120 320,320 80,320" fill="none" stroke="#f7f4ee" strokeWidth="1.2" opacity="0.9" />
      ...
      <text x="20" y="460" fill="#a8a197" fontFamily="monospace" fontSize="11" letterSpacing="2">TEXT → MOTION</text>
      <text x="20" y="478" fill="#6b665d" fontFamily="monospace" fontSize="9" letterSpacing="1">/prism/render_queue.py</text>
    </svg>
  ),
  potato: () => (
    <svg viewBox="0 0 400 500" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" style={{display:'block'}}>
      ...
      {Array.from({length: 6}).map((_, i) => Array.from({length: 6}).map((_, j) => {
        const cx = 40 + j * 60, cy = 60 + i * 60;
        const sick = (i + j) % 4 === 0;
        return (
          <g key={`${i}-${j}`}>
            <ellipse cx={cx} cy={cy} rx="22" ry="14" fill={sick ? '#8a6d3b' : '#3d5a2c'} opacity={sick ? 0.85 : 0.7} transform={`rotate(${(i*7+j*3)%30 - 15} ${cx} ${cy})`} />
            {sick && <circle cx={cx-4} cy={cy-2} r="3" fill="#141311" opacity="0.7" />}
          </g>
        );
      }))}
      <rect x="20" y="420" width="360" height="56" fill="#141311" />
      <text x="32" y="445" fill="#ffd60a" fontFamily="monospace" fontSize="11" letterSpacing="2">DETECTED · LATE BLIGHT</text>
      <text x="32" y="462" fill="#f7f4ee" fontFamily="monospace" fontSize="10">confidence: 0.942  ·  t=0.18s</text>
    </svg>
  ),
  inam: () => (
    <svg viewBox="0 0 400 500" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" style={{display:'block'}}>
      ...
      <text x="20" y="480" fill="#6b665d" fontFamily="monospace" fontSize="9" letterSpacing="1">FLUTTER · WINDOWS DESKTOP</text>
    </svg>
  ),
};
    ...
    <section id="work" className="sec reveal">
      <div className="container">
        <div className="sec-head">
          ...
          <h2 className="sec-title">Three projects, <br/><span className="ital">end-to-end</span>.</h2>
          <p className="sec-desc">From CNN architecture to cross-platform apps. Each one shipped, or currently in active development.</p>
        </div>
            ...
            return (
              <article className="project" key={p.id}>
                ...
                <div className="project-body">
                  ...
                  {p.repo && (
                    <a className="project-repo" href={p.repo} target="_blank" rel="noopener">
